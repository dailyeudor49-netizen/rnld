...
export default function Page() {
  ...
  const checkDuplicatePhone = (phone: string): boolean => {
    const STORAGE_KEY = 'unc_specchio_cz_submissions';
  ...
  const checkDeviceSubmitted = (): boolean => {
    return localStorage.getItem('unc_specchio_cz_device_submitted') === 'true';
  };
  ...
  const saveSubmission = (phone: string) => {
    ...
    localStorage.setItem('unc_specchio_cz_device_submitted', 'true');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (checkDuplicatePhone(formData.phone)) {
      setIsDuplicate(true);
      setFormErrors({ phone: 'Toto telefonní číslo je již registrováno. Brzy vás budeme kontaktovat!' });
      return;
    }
    if (checkDeviceSubmitted()) {
      setIsDuplicate(true);
      setFormErrors({ phone: 'Z tohoto zařízení již byla odeslána objednávka.' });
      return;
    }
