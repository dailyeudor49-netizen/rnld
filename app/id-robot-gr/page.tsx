...
export default function RobotAspirapolvereProLanding() {
  ...
  const checkDuplicatePhone = (phone: string): boolean => {
    const stored = localStorage.getItem('id_robot_gr_submissions');
  ...
  const checkDeviceSubmitted = (): boolean => {
    return localStorage.getItem('id_robot_gr_device_submitted') === 'true';
  };
  ...
  const saveSubmission = (phone: string) => {
    ...
    localStorage.setItem('id_robot_gr_submissions', JSON.stringify(submissions));
    localStorage.setItem('id_robot_gr_device_submitted', 'true');
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (checkDuplicatePhone(orderData.phone)) {
      setIsDuplicate(true);
      setSubmitError('Αυτός ο αριθμός τηλεφώνου υπάρχει ήδη στο σύστημά μας. Θα επικοινωνήσουμε σύντομα μαζί σας!');
      return;
    }

    if (checkDeviceSubmitted()) {
      setIsDuplicate(true);
      setSubmitError('Από αυτή τη συσκευή έχει ήδη γίνει παραγγελία. Θα επικοινωνήσουμε σύντομα μαζί σας!');
      return;
    }
