...
export default function Page() {
  ...
  const checkDuplicatePhone = (phone: string): boolean => {
    const STORAGE_KEY = 'id_specchio_hu_submissions';
  ...
  const checkDeviceSubmitted = (): boolean => {
    return localStorage.getItem('id_specchio_hu_device_submitted') === 'true';
  };
  ...
  const saveSubmission = (phone: string) => {
    ...
    localStorage.setItem('id_specchio_hu_device_submitted', 'true');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (checkDuplicatePhone(formData.phone)) {
      setIsDuplicate(true);
      setFormErrors({ phone: 'Ez a telefonszám már regisztrálva van. Hamarosan felvesszük Önnel a kapcsolatot!' });
      return;
    }
    if (checkDeviceSubmitted()) {
      setIsDuplicate(true);
      setFormErrors({ phone: 'Erről az eszközről már érkezett rendelés.' });
      return;
    }
