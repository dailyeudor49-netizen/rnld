...
export default function RobotAspirapolvereProLanding() {
  ...
  const checkDuplicatePhone = (phone: string): boolean => {
    const stored = localStorage.getItem('id_robot_si_submissions');
  ...
  const checkDeviceSubmitted = (): boolean => {
    return localStorage.getItem('id_robot_si_device_submitted') === 'true';
  };
  ...
  const saveSubmission = (phone: string) => {
    ...
    localStorage.setItem('id_robot_si_submissions', JSON.stringify(submissions));
    localStorage.setItem('id_robot_si_device_submitted', 'true');
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!orderData.name.trim() || !orderData.phone.trim() || !orderData.address.trim()) {
      setSubmitError('Prosimo, izpolnite vsa polja!');
      return;
    }

    if (checkDuplicatePhone(orderData.phone)) {
      setIsDuplicate(true);
      setSubmitError('Ta telefonska številka je že v našem sistemu. Kmalu vas bomo kontaktirali!');
      return;
    }

    if (checkDeviceSubmitted()) {
      setIsDuplicate(true);
      setSubmitError('Iz te naprave je bilo že oddano naročilo. Kmalu vas bomo kontaktirali!');
      return;
    }
    ...
    try {
      ...
      if (responseOk) {
        ...
        router.push('/ty/ty-id-robot-si');
      } else {
        setSubmitError('Prišlo je do napake. Poskusite znova.');
  ...
  const comparisonData = [
    { feature: "Sesanje + Pomivanje", robot: "OBOJE VKLJUČENO", without: "Samo sesanje" },
    { feature: "Praznjenje", robot: "AVTOMATSKO 60 DNI", without: "Ročno vsak dan" },
    { feature: "Dlake in lasje", robot: "ŠČETKA PROTI ZAPLETANJU", without: "Se zapletejo in blokirajo" },
    { feature: "Navigacija", robot: "LIDAR LASER 360°", without: "Naključna, trka povsod" },
    { feature: "Filter za alergije", robot: "HEPA H13 CERTIFICIRAN", without: "Osnovni filter" },
    { feature: "Skupna cena", robot: "€79 S POSTAJO", without: "€500+ za iste funkcije" },
  ];

  const faqs = [
    {
      question: "Kako deluje plačilo?",
      answer: "Plačate ob dostavi, v gotovini kurirju. Nič vam ni treba plačati vnaprej. Izpolnite obrazec, poklicali vas bomo za potrditev in paket boste prejeli v 24-48 urah. Brez tveganja."
    },
    {
      question: "Ga moram vsak dan prazniti?",
      answer: "Ne, postaja ga avtomatsko izprazni v 3-litrsko vrečko. Vrečko zamenjate vsaka 2 meseca. Nikoli se ne dotaknete prahu."
    },
    {
      question: "Deluje z živalskimi dlaki?",
      answer: "Da, ščetka proti zapletanju je zasnovana za dlake in dolge lase. Nikoli se ne zamaši. Več kot 800 strank s hišnimi ljubljenčki to potrjuje."
    },
    {
      question: "Kaj če mi ne bo všeč?",
      answer: "Imate 30 dni za vračilo. Polno povračilo, brezplačna vrnitev, brez vprašanj. Ampak 96% strank ga obdrži."
    }
  ];

  const reviews = [
    { nome: 'Marko K.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Trije veliki psi. Sesala sem VSAK DAN. S tem robotom sesam enkrat TEDENSKO za kote. 90% dela opravi on. Najboljši nakup zadnjih 5 let.', stelle: 5, data: 'pred 5 dnevi', risposta: 'Marko, hvala! Lastniki hišnih ljubljenčkov so naše najbolj zadovoljne stranke.' },
    { nome: 'Ana N.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Mož je bil skeptičen. "Za to ceno bo igrača". Po 2 tednih se mi je opravičil. Čisti BOLJE kot naš stari Roomba za €600. Postaja s samodejnim praznjenjem je genialna.', stelle: 5, data: 'pred 1 tednom' },
    { nome: 'Peter S.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Stanovanje 85m² na dveh nadstropjih. Nosim ga gor-dol in vse opravi sam. Baterija je neskončna, nikoli ga ni bilo treba ustaviti sredi dela. Zelo tih, vklopim ga med delom od doma.', stelle: 5, data: 'pred 3 dnevi', risposta: 'Peter, točno tako! 5200mAh baterija zagotavlja do 6 ur!' },
    { nome: 'Mojca L.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Imam 68 let s težavami s hrbtom. Ne morem več sesati. Ta robot mi je spremenil življenje. Programiram ga in on vse naredi. Končno lahko uživam v čistem domu brez bolečin.', stelle: 5, data: 'pred 4 dnevi' },
    { nome: 'Tomaž V.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Alergičen na pršice od vedno. Odkar uporabljam ta robot s HEPA filtrom, se zbujam brez zamašenega nosu. Razlika je OGROMNA. Moral bi ga kupiti že leta prej.', stelle: 5, data: 'pred 2 tednoma', risposta: 'Tomaž, HEPA H13 filter res naredi razliko za tiste, ki trpijo za alergijami!' },
    { nome: 'Nina U.', paese: 'Avstrija', flag: '🇦🇹', testo: 'Primerjala sem to z Roborockom za €900 pri svaku. ISTE FUNKCIJE. Ni mogel verjeti. Pokazala sem mu postajo, ki prazni in pomiva. Zdaj tudi on hoče enega.', stelle: 5, data: 'pred 1 tednom' },
    { nome: 'Sabina M.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Hiša s 3 mačkami. Dlake so bile nočna mora. Zdaj robot gre 2x na dan in hiša je vedno popolna. Gostje ne verjamejo, da imam 3 mačke. NAJBOLJŠE DARILO, ki sem si ga naredila.', stelle: 5, data: 'pred 6 dnevi' },
    { nome: 'Janez H.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Dolga zima, hiša vedno zaprta. Prah se je hitro nabiral. Zdaj robot čisti vsak dan, ko sem v službi. Pridem domov in diham čist zrak. Vredno vsake pare.', stelle: 5, data: 'pred 10 dnevi', risposta: 'Janez, točno tako! Odličen za slovenske zime.' },
    { nome: 'Maja T.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Dolgi lasje povsod, bila mi je nočna mora. Ta robot posesa VSE brez zapletanja. Ščetka se nikoli ne blokira. Probala sem 3 robote pred tem, končno eden, ki deluje!', stelle: 5, data: 'pred 8 dnevi' },
    { nome: 'Matej B.', paese: 'Slovenija', flag: '🇸🇮', testo: 'Uporabljam ga že 2 meseca, vsak dan. Nič problemov. Kakovost je identična robotom za €1000. Prihranil sem celo premoženje in imam vedno čist dom. Priporočam vsem.', stelle: 5, data: 'pred 12 dnevi', risposta: 'Matej, hvala za zaupanje! Kakovost govori sama zase.' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden font-sans leading-relaxed">
      ...
      {/* Click Pixel */}
      <img
        src="https://offers.italiadrop.com/forms/api/ck/?o=2368&uid=019be502-1631-773c-b833-f6153c79c2cb&lp=2407"
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t-2 border-blue-600 shadow-2xl">
        <div className="max-w-md mx-auto px-4 py-2">
          <div className="flex items-center justify-between mb-1">
            <span className="text-red-600 font-bold text-sm animate-pulse">Samo še {stockLeft}!</span>
          ...
          >
            <span>NAROČI ZDAJ - PLAČILO OB DOSTAVI</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-red-600 text-white py-2 text-center font-bold text-sm px-4">
        <div className="flex items-center justify-center gap-2">
          ...
          <span>BLISKOVITA PONUDBA - Poteče čez: {formatTime(timeLeft)}</span>
        </div>
      </div>
      ...
      <section className="bg-gradient-to-b from-gray-900 to-gray-800 pt-6 pb-4 md:py-8 px-4 text-white">
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-red-500 text-white text-sm md:text-base font-bold py-2 px-4 rounded-full inline-block mb-4 animate-pulse">
            TOTALNA RAZPRODAJA — ZADNJIH {stockLeft} KOSOV
          </div>
          <h1 className="text-2xl md:text-4xl font-black mb-4 leading-tight tracking-tight">
            SESA, POMIVA IN SE IZPRAZNI<br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Z ENIM PREHODOM</span>
          </h1>
          <p className="text-lg md:text-xl mb-4 text-gray-300">
            Vrzite stran krpo, vedro in metlo. <span className="font-bold text-white">On naredi vse sam.</span>
          </p>
          <div className="bg-white/10 backdrop-blur border border-white/20 rounded-xl p-4 inline-block">
            ...
            <span className="block text-green-400 font-bold mt-1">Postaja za avtomatsko praznjenje VKLJUČENA (vrednost €199)</span>
          </div>
        </div>
      </section>
            ...
            <div className="bg-gray-50 rounded-xl p-5 text-left lg:hidden border border-gray-200">
              ...
              <ul className="space-y-3 text-gray-700 text-base">
                <li className="flex items-start gap-3">
                  ...
                  <span><strong className="text-gray-800">Sesa in pomiva skupaj</strong> — En prehod, popolna tla.</span>
                </li>
                <li className="flex items-start gap-3">
                  <Trash2 className="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5" />
                  <span><strong className="text-gray-800">Robot se sam izprazni v postajo</strong> — Vrečko zamenjate vsaka 2 meseca.</span>
                </li>
                <li className="flex items-start gap-3">
                  <Dog className="w-6 h-6 text-amber-600 flex-shrink-0 mt-0.5" />
                  <span><strong className="text-gray-800">Adijo dlake in lasje</strong> — Ščetka proti zapletanju, nikoli se ne blokira.</span>
                </li>
                <li className="flex items-start gap-3">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <span><strong className="text-gray-800">Laserska navigacija</strong> — Kartira dom, ne trka v pohištvo.</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="lg:sticky lg:top-4 lg:self-start">
            ...
            <div className="bg-white border-2 border-gray-200 rounded-2xl p-5 mb-6 shadow-xl relative overflow-hidden">
              ...
              <h2 className="text-xl font-black text-gray-900 mb-1">
                NovaClean X1 PRO + OMNI Postaja
              </h2>
              <p className="text-sm text-gray-500 mb-2">Profesionalni robotski sesalnik in čistilec tal</p>
              ...
              <div className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded">PRIHRANITE €120,90</span>
                </div>
              ...
              <div className="flex items-center justify-center gap-2 mt-3 text-red-600 font-bold">
                ...
                <span>Samo še {stockLeft} kosov po tej ceni!</span>
              </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white rounded-2xl p-5 shadow-md text-center">
              ...
              <h4 className="font-black text-gray-900 mb-1">Adijo Bolečine v Hrbtu</h4>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-md text-center">
              ...
              <p className="text-gray-500 text-sm">Robot se sam izprazni v postajo. Nikoli se ne dotaknete prahu.</p>
            </div>
            <div className="bg-white rounded-2xl p-5 shadow-md text-center">
              <div className="text-3xl mb-2">🐕</div>
              <h4 className="font-black text-gray-900 mb-1">Živalske Dlake? Izginile</h4>
              <p className="text-gray-500 text-sm">Ščetka proti zapletanju. Nikoli se ne zamaši.</p>
            </div>
            <div className="bg-white rounded-2xl p-5 shadow-md text-center">
              <div className="text-3xl mb-2">💨</div>
          ...
          <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl p-6 md:p-8 text-white text-center shadow-2xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white/10 rounded-xl p-3 text-center">
                ...
                <p className="text-xs text-gray-300">Močno sesanje</p>
              </div>
              <div className="bg-white/10 rounded-xl p-3 text-center">
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-10">
            <div className="inline-block bg-blue-600 text-white text-sm font-bold px-4 py-1 rounded-full mb-4">
              VKLJUČENA V CENO (VREDNOST €199)
            </div>
            <h2 className="text-3xl md:text-5xl font-black mb-4 text-gray-900">
              POSTAJA, KI <span className="text-blue-600">ELIMINIRA VSE VAŠE DELO</span>
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Robot se vrne v postajo, se izprazni, opere krpo, napolni. Vaša nova osebna čistilka.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 items-start">
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                <img src="/images/robot-asp/svuota.webp" alt="Avtomatsko praznjenje" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                {/* Desktop overlay */}
                <div className={`hidden md:flex absolute inset-0 items-end transition-all duration-500 ${openFeature === 0 ? 'bg-black/85' : 'bg-gradient-to-t from-black/70 via-black/20 to-transparent'}`}>
                  <div className="p-4 text-white w-full">
                    <div className="flex items-center justify-between">
                      <p className="font-black text-xl leading-tight">ROBOT SE SAM IZPRAZNI V POSTAJO</p>
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 0 ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                      <p className="text-sm text-white/90 leading-relaxed">
                        Vrne se v postajo in se SAM izprazni v 3-litrsko vrečko. <strong>Vrečko zamenjate vsaka 2 meseca.</strong> Nikoli več rok v prahu. Nikoli več kihanja.
                      </p>
                    </div>
              ...
              <div className="md:hidden p-3 bg-gradient-to-r from-blue-600 to-indigo-700">
                <div className="flex items-center justify-between">
                  <p className="font-black text-sm text-white leading-tight">ROBOT SE SAM IZPRAZNI V POSTAJO</p>
                ...
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 0 ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    Vrne se v postajo in se SAM izprazni v 3-litrsko vrečko. Vrečko zamenjate vsaka 2 meseca. Nikoli več rok v prahu.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                <img src="/images/robot-asp/mocio.webp" alt="Avtomatsko pranje" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                {/* Desktop overlay */}
                <div className={`hidden md:flex absolute inset-0 items-end transition-all duration-500 ${openFeature === 1 ? 'bg-black/85' : 'bg-gradient-to-t from-black/70 via-black/20 to-transparent'}`}>
                  <div className="p-4 text-white w-full">
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 1 ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                      <p className="text-sm text-white/90 leading-relaxed">
                        Po vsakem pomivanju <strong>postaja opere krpo</strong> s čisto vodo. Vi se ne dotaknete ničesar. Tla vedno popolna.
                      </p>
                    </div>
                    <p className={`text-sm text-white/80 transition-all duration-300 ${openFeature === 1 ? 'opacity-0 h-0' : 'opacity-100 mt-1'}`}>Postaja jo opere namesto vas</p>
                  </div>
                </div>
              </div>
              {/* Mobile text outside */}
              <div className="md:hidden p-3 bg-gradient-to-r from-blue-600 to-indigo-700">
                ...
                <p className={`text-sm text-blue-100 transition-all duration-300 ${openFeature === 1 ? 'hidden' : 'mt-1'}`}>Postaja jo opere namesto vas</p>
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 1 ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    Po vsakem pomivanju postaja opere krpo s čisto vodo. Vi se ne dotaknete ničesar. Tla vedno popolna.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                <img src="/images/robot-asp/muffa.webp" alt="Sušenje na toplo" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                {/* Desktop overlay */}
                <div className={`hidden md:flex absolute inset-0 items-end transition-all duration-500 ${openFeature === 2 ? 'bg-black/85' : 'bg-gradient-to-t from-black/70 via-black/20 to-transparent'}`}>
                  <div className="p-4 text-white w-full">
                    <div className="flex items-center justify-between">
                      <p className="font-black text-xl leading-tight">ADIJO VONJ IN PLESEN</p>
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 2 ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                      <p className="text-sm text-white/90 leading-relaxed">
                        Posuši krpo na <strong>45°C</strong> po vsakem pomivanju. Brez bakterij, brez neprijetnih vonjav. <strong>PRAVA higiena.</strong>
                      </p>
                    </div>
                    <p className={`text-sm text-white/80 transition-all duration-300 ${openFeature === 2 ? 'opacity-0 h-0' : 'opacity-100 mt-1'}`}>Hitro sušenje na 45°C</p>
                  </div>
                </div>
              </div>
              {/* Mobile text outside */}
              <div className="md:hidden p-3 bg-gradient-to-r from-blue-600 to-indigo-700">
                <div className="flex items-center justify-between">
                  <p className="font-black text-sm text-white leading-tight">ADIJO VONJ IN PLESEN</p>
                  <ChevronDown className={`w-4 h-4 text-white transition-transform duration-300 ${openFeature === 2 ? 'rotate-180' : ''}`} />
                </div>
                <p className={`text-sm text-blue-100 transition-all duration-300 ${openFeature === 2 ? 'hidden' : 'mt-1'}`}>Hitro sušenje na 45°C</p>
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 2 ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    Posuši krpo na 45°C po vsakem pomivanju. Brez bakterij, brez neprijetnih vonjav. PRAVA higiena.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
                ...
                <div className={`hidden md:flex absolute inset-0 items-end transition-all duration-500 ${openFeature === 3 ? 'bg-black/85' : 'bg-gradient-to-t from-black/70 via-black/20 to-transparent'}`}>
                  <div className="p-4 text-white w-full">
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 3 ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                      <p className="text-sm text-white/90 leading-relaxed">
                        Kartira celoten dom z milimetrsko natančnostjo. <strong>Izogiba se oviram</strong>, ne trka v pohištvo. Prazna baterija? <strong>Se napolni in nadaljuje sam.</strong>
                      </p>
                    </div>
                    <p className={`text-sm text-white/80 transition-all duration-300 ${openFeature === 3 ? 'opacity-0 h-0' : 'opacity-100 mt-1'}`}>Kartira, se izogiba oviram, nikoli ne ustavi</p>
                  </div>
                </div>
              </div>
              {/* Mobile text outside */}
              <div className="md:hidden p-3 bg-gradient-to-r from-blue-600 to-indigo-700">
                ...
                <p className={`text-sm text-blue-100 transition-all duration-300 ${openFeature === 3 ? 'hidden' : 'mt-1'}`}>Kartira, se izogiba oviram</p>
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 3 ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    Kartira celoten dom z milimetrsko natančnostjo. Izogiba se oviram, ne trka v pohištvo. Se napolni in nadaljuje sam.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                <img src="/images/robot-asp/6.webp" alt="Dlake in lasje" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                {/* Desktop overlay */}
                <div className={`hidden md:flex absolute inset-0 items-end transition-all duration-500 ${openFeature === 4 ? 'bg-black/85' : 'bg-gradient-to-t from-black/70 via-black/20 to-transparent'}`}>
                  <div className="p-4 text-white w-full">
                    <div className="flex items-center justify-between">
                      <p className="font-black text-xl leading-tight">ADIJO DLAKE IN LASJE</p>
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 4 ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                      <p className="text-sm text-white/90 leading-relaxed">
                        Ščetka proti zapletanju zasnovana za <strong>živalske dlake, dolge lase in trdovratno umazanijo</strong>. Nikoli se ne zamaši. Posesa vse ob prvem prehodu.
                      </p>
                    </div>
                    <p className={`text-sm text-white/80 transition-all duration-300 ${openFeature === 4 ? 'opacity-0 h-0' : 'opacity-100 mt-1'}`}>Ščetka proti zapletanju, nikoli se ne blokira</p>
                  </div>
                </div>
              </div>
              {/* Mobile text outside */}
              <div className="md:hidden p-3 bg-gradient-to-r from-blue-600 to-indigo-700">
                <div className="flex items-center justify-between">
                  <p className="font-black text-sm text-white leading-tight">ADIJO DLAKE IN LASJE</p>
                  <ChevronDown className={`w-4 h-4 text-white transition-transform duration-300 ${openFeature === 4 ? 'rotate-180' : ''}`} />
                </div>
                <p className={`text-sm text-blue-100 transition-all duration-300 ${openFeature === 4 ? 'hidden' : 'mt-1'}`}>Ščetka proti zapletanju</p>
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 4 ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    Ščetka proti zapletanju za živalske dlake, dolge lase in trdovratno umazanijo. Nikoli se ne zamaši. Posesa vse ob prvem prehodu.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                <img src="/images/robot-asp/divano.webp" alt="Upravljanje z aplikacijo" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                {/* Desktop overlay */}
                <div className={`hidden md:flex absolute inset-0 items-end transition-all duration-500 ${openFeature === 5 ? 'bg-black/85' : 'bg-gradient-to-t from-black/70 via-black/20 to-transparent'}`}>
                  <div className="p-4 text-white w-full">
                    <div className="flex items-center justify-between">
                      <p className="font-black text-xl leading-tight">UPRAVLJANJE S KAVČA</p>
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 5 ? 'max-h-40 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                      <p className="text-sm text-white/90 leading-relaxed">
                        Daljinec vključen. Aplikacija po želji. <strong>"Alexa, počisti doma"</strong> in začne. Vi sploh ne vstanete.
                      </p>
                    </div>
                    <p className={`text-sm text-white/80 transition-all duration-300 ${openFeature === 5 ? 'opacity-0 h-0' : 'opacity-100 mt-1'}`}>Aplikacija, Alexa, daljinec</p>
                  </div>
                </div>
              </div>
              {/* Mobile text outside */}
              <div className="md:hidden p-3 bg-gradient-to-r from-blue-600 to-indigo-700">
                <div className="flex items-center justify-between">
                  <p className="font-black text-sm text-white leading-tight">UPRAVLJANJE S KAVČA</p>
                  <ChevronDown className={`w-4 h-4 text-white transition-transform duration-300 ${openFeature === 5 ? 'rotate-180' : ''}`} />
                </div>
                <p className={`text-sm text-blue-100 transition-all duration-300 ${openFeature === 5 ? 'hidden' : 'mt-1'}`}>Aplikacija, Alexa, daljinec</p>
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openFeature === 5 ? 'max-h-40 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    Daljinec vključen. Aplikacija po želji. "Alexa, počisti doma" in začne. Vi sploh ne vstanete.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <p className="text-center text-gray-500 mb-8">
            Primerjajte ga s top modeli: ni konkurence.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-center">
              ...
              <p className="text-sm text-gray-500">Moč sesanja</p>
            </div>
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-center">
      ...
      <section className="py-12 bg-gray-50">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <p className="text-lg text-gray-600 text-center mb-8">
            Iste funkcije, €400 manj. Izračunajte sami.
          </p>
      ...
      <section className="bg-red-600 py-6">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <p className="text-white text-xl font-black mb-3">
            Totalna razprodaja — Samo še {stockLeft} kosov
          </p>
          <p className="text-white/80 mb-4">Ko zmanjka, se cena vrne na €199,90. Postaja vključena samo za te zadnje kose.</p>
          <button
            onClick={openOrderPopup}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {reviews.map((review, i) => (
              <div key={i} className={`bg-white rounded-xl p-5 shadow ${i >= visibleReviews ? 'hidden md:block' : ''}`}>
                ...
                <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded font-medium">Potrjen nakup</span>
                {review.risposta && (
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left">
              <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-3">
                VAM NI VŠEČ? VRNEMO VAM VSE. KONEC.
              </h2>
              <p className="text-gray-700 mb-4 text-lg">
                Imate 30 dni, da ga preizkusite. Če niste NAVDUŠENI — iz kakršnega koli razloga — nas pokličite, pošljemo kurirja po njega in vam vrnemo vsak cent. <strong>Vračilo BREZPLAČNO. Brez vprašanj. Brez tveganja za vas.</strong> Ampak 96% strank ga obdrži.
              </p>
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex flex-row items-start justify-center gap-2 md:gap-8">
            ...
            <div className="flex flex-col items-center text-center flex-1">
              ...
              <p className="text-gray-700 text-sm font-medium">Poklicali vas bomo za potrditev</p>
            </div>
            <div className="text-gray-300 text-2xl mt-4">→</div>
            <div className="flex flex-col items-center text-center flex-1">
              ...
              <p className="text-gray-700 text-sm font-medium">Prejmete v 24-48 urah in plačate KURIRJU!</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="order-form-section" className="bg-gray-900 py-12 pb-8">
        <div className="max-w-xl mx-auto px-4">
          <div className="bg-red-600 text-white font-bold text-center py-2 rounded-full mb-4 animate-pulse">
            Samo še {stockLeft} kosov po tej ceni
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-2xl">
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-4 border border-gray-200">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-600">+ OMNI Postaja + Dodatki</p>
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 mb-5 text-center">
              <div className="flex items-center justify-center gap-2 text-red-700 font-bold">
                ...
                <span>Ponudba poteče čez: {formatTime(timeLeft)}</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-4 mb-5">
                ...
                <div>
                  <label className="block text-gray-800 font-bold mb-2">Polni Naslov *</label>
                  <input
                    type="text"
                    name="address"
              ...
              >
                {isSubmitting ? 'POŠILJAM...' : 'NAROČI ZDAJ — PLAČILO OB DOSTAVI'}
                {!isSubmitting && <Truck className="w-6 h-6" />}
              </button>

              <p className="text-center text-gray-400 text-xs mt-4">
                Vaši podatki so zaščiteni in SSL šifrirani. Uporabljamo jih SAMO za dostavo.
              </p>
            </form>
          </div>
        </div>
