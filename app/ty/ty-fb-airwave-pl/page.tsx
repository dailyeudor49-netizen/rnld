...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      if (status === 'failed') {
        // Lead failed on landing page, show error and try retry
        setPopup('error');
        ...
        // Lead succeeded on first try, no popup needed
        clearLeadStatus();
      } else {
        // No status, just try retry in case there are old failed leads
        const retrySuccess = await retryFailedLeads();
        if (retrySuccess) {
          setPopup('success');
        }
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ color: '#6B7280', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              Wystąpił problem z potwierdzeniem zamówienia. Proszę odświeżyć stronę za chwilę, aż pojawi się potwierdzenie.
            </p>
            <button
      ...
      {popup === 'success' && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ color: '#6B7280', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              Przepraszamy za wcześniejsze niedogodności. Twoje zamówienie zostało pomyślnie potwierdzone.
            </p>
            <button
