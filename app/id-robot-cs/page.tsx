...
export default function RobotAspirapolvereProLanding() {
  ...
  const checkDuplicatePhone = (phone: string): boolean => {
    const stored = localStorage.getItem('id_robot_cs_submissions');
  ...
  const checkDeviceSubmitted = (): boolean => {
    return localStorage.getItem('id_robot_cs_device_submitted') === 'true';
  };
  ...
  const saveSubmission = (phone: string) => {
    ...
    localStorage.setItem('id_robot_cs_submissions', JSON.stringify(submissions));
    localStorage.setItem('id_robot_cs_device_submitted', 'true');
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (checkDuplicatePhone(orderData.phone)) {
      setIsDuplicate(true);
      setSubmitError('Toto telefonní číslo je již v našem systému. Brzy vás budeme kontaktovat!');
      return;
    }

    if (checkDeviceSubmitted()) {
      setIsDuplicate(true);
      setSubmitError('Z tohoto zařízení již byla odeslána objednávka. Brzy vás budeme kontaktovat!');
      return;
    }
