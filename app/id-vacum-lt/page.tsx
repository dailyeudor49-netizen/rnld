    ...
    return (
      <div className="py-12 px-4 bg-slate-50 text-center">
        <div className="max-w-lg mx-auto bg-white rounded-xl shadow-xl p-8 border-4 border-green-500">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 mb-4">Ačiū {formData.name}!</h2>
          <p className="text-xl text-slate-600 mb-6">
            Gavome jūsų užklausą.<br/>
            Mūsų konsultantas paskambins numeriu <strong>{formData.phone}</strong>, kad patvirtintų detales.
          </p>
          <button onClick={() => window.location.reload()} className="text-blue-600 font-bold underline text-lg">Grįžti į pagrindinį puslapį</button>
        </div>
      </div>
    );
  }
