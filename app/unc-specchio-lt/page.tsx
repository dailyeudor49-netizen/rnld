...
export default function Page() {
  ...
  const checkDuplicatePhone = (phone: string): boolean => {
    const STORAGE_KEY = 'unc_specchio_lt_submissions';
  ...
  const checkDeviceSubmitted = (): boolean => {
    return localStorage.getItem('unc_specchio_lt_device_submitted') === 'true';
  };
  ...
  const saveSubmission = (phone: string) => {
    ...
    localStorage.setItem('unc_specchio_lt_device_submitted', 'true');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (checkDuplicatePhone(formData.phone)) {
      setIsDuplicate(true);
      setFormErrors({ phone: 'Šis telefono numeris jau užregistruotas. Netrukus su jumis susisieksime!' });
      return;
    }
    if (checkDeviceSubmitted()) {
      setIsDuplicate(true);
      setFormErrors({ phone: 'Iš šio įrenginio užsakymas jau buvo pateiktas.' });
      return;
    }
    ...
    } catch { saveUserDataToStorage({ nome: formData.fullName, cognome: '', telefono: formData.phone, indirizzo: formData.addressFull }); window.location.href = '/ty/ty-unc-specchio-lt'; }
